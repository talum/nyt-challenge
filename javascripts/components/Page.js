...
Page.prototype.render = function(state) {
  ...
  var detailHTML = `<div class="heading heading--level-1">${this.shouldMartianize ? this.martianize(this.details.title) : this.details.title}</div>
    <div class="heading heading--level-2">${this.shouldMartianize ? this.martianize(this.details.description) : this.details.description}</div>
    <button class="button button--color-black util--margin-lm" onclick="NYTD.toggleLanguage()">${this.shouldMartianize ? this.martianize(buttonText) : buttonText}</button>`
  var self = this
